package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class SpacesNewCatalogWidget extends ItemGridCatalogWidget implements IDisposable, ICatalogWidget, IItemGrid
   {
      ...
      private var var_461:ISelectorWindow;
      ...
      public function SpacesNewCatalogWidget(param1:IWindowContainer)
      {
         ...
         this.var_584 = [0,0,0];
         this.var_102 = ["group.walls","group.floors","group.views"];
         super(param1);
         this.var_461 = _window.findChildByName("groups") as ISelectorWindow;
         if(this.var_461)
         {
            _loc2_ = 0;
            while(_loc2_ < this.var_461.numSelectables)
            {
               _loc3_ = this.var_461.getSelectableAt(_loc2_);
      ...
      override public function init() : Boolean
      {
         ...
         this.switchCategory(this.var_102[this.var_491]);
      ...
      {
         var _loc2_:int = this.var_584[this.var_491];
         var _loc3_:Offer = this.var_198[this.var_491][_loc2_];
      ...
      {
         var _loc2_:* = null;
         if(param1 > -1 && param1 < var_482.numGridItems)
         {
            _loc2_ = this.var_198[this.var_491][param1];
      ...
      override public function select(param1:IGridItem) : void
      {
         ...
         this.var_584[this.var_491] = (this.var_198[this.var_491] as Array).indexOf(_loc2_);
         this.updateRoomPreview();
      }
      
      private function updateRoomPreview() : void
      {
         var _loc1_:int = this.var_584[0];
         var _loc2_:int = this.var_584[1];
         var _loc3_:int = this.var_584[2];
         var _loc4_:Offer = this.var_198[0][_loc1_];
         var _loc5_:Offer = this.var_198[1][_loc2_];
         var _loc6_:Offer = this.var_198[2][_loc3_];
      ...
      private function createOfferGroups() : Boolean
      {
         ...
         {
            if(!(_loc1_.pricingModel == Offer.const_396 || _loc1_.pricingModel == Offer.const_438))
            {
               continue;
            }
            ...
            if(!(_loc2_.productType == ProductTypeEnum.const_66 || _loc2_.productType == ProductTypeEnum.const_77))
            {
               continue;
            }
            ...
            while(this.var_198.length < this._groupNames.length)
            {
               this.var_198.push(new Array());
            }
            switch(_loc4_)
            {
               case "floor":
                  (this.var_198[_loc5_] as Array).push(_loc1_);
                  break;
               case "wallpaper":
                  (this.var_198[_loc5_] as Array).push(_loc1_);
                  break;
      ...
      private function onSelectGroup(param1:WindowEvent) : void
      {
         ...
         if(_loc2_)
         {
            _loc3_ = this.var_461.getSelectableIndex(_loc2_);
      ...
      private function switchCategory(param1:String) : void
      {
         ...
         this.var_461.setSelected(this.var_461.getSelectableByName(param1));
         var _loc2_:int = -1;
         switch(param1)
         {
            case "group.walls":
         ...
         if(_loc2_ > -1)
         {
            ...
            if(var_482)
            {
               var_482.destroyGridItems();
            }
            page.replaceOffers(this.var_198[this.var_491],false);
            resetTimer();
            populateItemGrid();
            ...
            _loc3_ = this.var_584[this.var_491];
