package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
         ...
         try
         {
            ...
            for each(updatedFriend in p.updatedFriends)
            {
               ...
               if(currentlyOnline && !updatedFriend.online)
               {
                  ...
                  this.var_471.notifications.addOfflineNotification(updatedFriend.name,updatedFriend.realName);
               }
               if(!currentlyOnline && updatedFriend.online)
               {
                  this.var_471.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_471.view.setNewMessageArrived();
                  this.var_471.notifications.addOnlineNotification(updatedFriend.name,updatedFriend.figure,updatedFriend.realName);
               }
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         this.findCategory(FriendCategory.const_1198).received = true;
         ...
         for each(_loc3_ in this.getCategoriesNotReceived())
         {
            ...
            {
               Util.remove(this.var_102,_loc3_);
               _loc3_.dispose();
            }
         }
      }
