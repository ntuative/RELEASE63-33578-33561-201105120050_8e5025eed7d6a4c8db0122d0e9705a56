package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private var var_353:IBitmapWrapperWindow;
      ...
      private function createView() : void
      {
         ...
         this.var_353 = this._view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(this.var_623 && this.var_1433)
         {
            _loc5_ = this._controller.avatarRenderManager.createAvatarImage(this.figure,AvatarScaleType.const_95,this.var_936,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_35,parseInt(FigureData.const_827));
         ...
         if(this.var_353)
         {
            if(this.var_353.bitmap)
            {
               this.var_353.bitmap.dispose();
            }
            this.var_353.bitmap = new BitmapData(this.var_353.width,this.var_353.height,true,0);
            _loc7_ = (this.var_353.width - _loc1_.width) / 2;
            _loc8_ = (this.var_353.height - _loc1_.height) / 2;
            this.var_353.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
         ...
         if(_loc4_)
         {
            _loc4_.visible = this.var_1433 && this.var_623 != null;
         }
      }
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         switch(param2.name)
         {
            case "set_button":
               ...
               this.var_936 = this._controller.gender;
               this._controller.handler.saveWardrobeOutfit(this.var_1787,this);
               this.updateView();
               break;
            case "get_button":
               if(this.var_623)
               {
                  this._controller.loadAvatarInEditor(this.var_623,this.var_936,this._controller.clubMemberLevel);
               }
         }
      }
