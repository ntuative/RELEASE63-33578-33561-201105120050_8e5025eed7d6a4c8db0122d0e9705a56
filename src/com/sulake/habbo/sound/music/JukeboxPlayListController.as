package com.sulake.habbo.sound.music
{
   ...
   public class JukeboxPlayListController implements IPlayListController
   {
      ...
      {
         this.var_221.stop(this.priority);
         this.var_448 = -1;
         this.var_1662 = -1;
         this.var_650 = false;
      }
      ...
      {
         if(param1 < 0 || param1 >= this.var_489.length)
         {
            return null;
         }
         return this.var_489[param1];
      }
      ...
      private function onNowPlayingMessage(param1:IMessageEvent) : void
      {
         ...
         {
            this.var_221.playSong(_loc3_.currentSongId,HabboMusicPrioritiesEnum.const_124,_loc3_.syncCount / 1000,0,1,1);
            this.var_448 = _loc3_.currentSongId;
         }
         else
         {
            this.stopPlaying();
         ...
         {
            this.var_221.addSongInfoRequest(_loc3_.nextSongId);
         }
         this.var_1662 = _loc3_.currentPosition;
         this._soundManager.events.dispatchEvent(new NowPlayingEvent(NowPlayingEvent.const_101,HabboMusicPrioritiesEnum.const_124,_loc3_.currentSongId,_loc3_.currentPosition));
      }
      ...
      private function onJukeboxSongDisksMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc4_ < _loc3_.songDisks.length)
         {
            ...
            _loc7_ = this.var_221.getSongInfo(_loc5_) as SongDataEntry;
            if(_loc7_ == null)
            {
               ...
               if(this.var_715.indexOf(_loc5_) < 0)
               {
                  ...
                  this.var_221.requestSongInfoWithoutSamples(_loc5_);
               }
            }
            _loc7_.diskId = _loc6_;
            this.var_489.push(_loc7_);
            _loc4_++;
         }
      ...
      private function onSongInfoReceivedEvent(param1:SongInfoReceivedEvent) : void
      {
         ...
         {
            _loc4_ = this.var_489[_loc2_];
            if(_loc4_.id == param1.id)
            {
               _loc5_ = _loc4_.diskId;
               _loc6_ = this.var_221.getSongInfo(param1.id) as SongDataEntry;
               if(_loc6_ != null)
               {
                  _loc6_.diskId = _loc5_;
                  this.var_489[_loc2_] = _loc6_;
               }
               break;
            }
            _loc2_++;
         }
         var _loc3_:int = this.var_715.indexOf(param1.id);
         if(_loc3_ >= 0)
         {
            this.var_715.splice(_loc3_,1);
         }
         if(this.var_715.length == 0)
         {
            this._events.dispatchEvent(new PlayListStatusEvent(PlayListStatusEvent.const_104));
