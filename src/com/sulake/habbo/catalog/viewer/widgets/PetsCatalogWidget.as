package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_505 = this.getPetTypeIndexFromProduct(_loc3_.localizationId);
         if(this.var_505 >= 8)
         {
            return false;
         }
         this.updateAvailableBreeds(_loc3_.localizationId);
         this.var_2397 = _loc3_.localizationId;
         this.var_396 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc5_ = _loc4_.avatarRenderManager.petDataManager.getPetData(this.var_505,this);
               if(_loc5_)
               {
                  this.var_396 = _loc5_.colors;
         ...
         this._offers.add(this.var_505,_loc3_);
         events.addEventListener(WidgetEvent.CWE_SELECT_PRODUCT,this.onSelectProduct);
         events.addEventListener(WidgetEvent.CWE_COLOUR_INDEX,this.onColourIndex);
         events.addEventListener(WidgetEvent.const_812,this.onApproveNameResult);
      ...
      private function onWidgetsInitialized(param1:CatalogWidgetsInitializedEvent = null) : void
      {
         ...
         var _loc3_:* = [];
         for each(_loc4_ in this.var_396)
         {
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_553 = param1.index;
         if(this.var_553 < 0 || this.var_553 > this.var_396.length)
         {
            this.var_553 = 0;
         }
         this.updateImage();
      ...
      {
         if(param1.productCode == this.var_2397)
         {
      ...
      private function parseSellableBreeds(param1:Array) : Array
      {
         ...
         {
            if(_loc3_.type == this.var_505 && _loc3_.sellable)
            {
      ...
      private function updateBreedSelections() : void
      {
         ...
         while(_loc3_ < this.var_212.length)
         {
            ...
            _loc2_.push(_loc1_.getKey(this.getRaceLocalizationKey(this.var_505,_loc5_),this.getRaceLocalizationKey(this.var_505,_loc5_)));
            _loc3_++;
         }
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_212 == null || this.var_841 >= this.var_212.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_553 >= 0 && this.var_553 < this.var_396.length)
         {
            _loc7_ = this.var_396[this.var_553] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_212[this.var_841] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_505,_loc3_,_loc2_,AvatarScaleType.const_55,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_317,AvatarAction.const_797);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_553 >= this.var_396.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_396[this.var_553] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_212[this.var_841] as int;
