package com.sulake.habbo.sound.trax
{
   ...
   public class TraxSequencer implements IHabboSound, IDisposable
   {
      ...
      private static const const_185:uint = 8192;
      
      private static const const_1417:uint = 88000;
      
      private static const const_1418:uint = 88000;
      
      private static const const_295:Vector.<int> = new Vector.<int>(const_185,true);
      ...
      private var var_934:Sound;
      
      private var var_430:SoundChannel;
      
      private var var_1156:TraxData;
      ...
      {
         this.var_205 = uint(param1 * const_113);
      }
      ...
      {
         return this.var_427 / const_113;
      }
      
      public function set fadeOutSeconds(param1:Number) : void
      {
         this.var_427 = int(param1 * const_113);
      }
      
      public function get fadeInSeconds() : Number
      {
         return this.var_748 / const_113;
      }
      
      public function set fadeInSeconds(param1:Number) : void
      {
         this.var_748 = int(param1 * const_113);
      }
      ...
      private function prepareSequence() : Boolean
      {
         ...
         var _loc2_:int = 0;
         while(_loc2_ < this.var_1156.channels.length)
         {
            _loc3_ = new Map();
            _loc4_ = this.var_1156.channels[_loc2_] as TraxChannel;
            _loc5_ = 0;
            _loc6_ = 0;
            _loc7_ = 0;
            while(_loc7_ < _loc4_.itemCount)
            {
               ...
               _loc9_ = this.var_1785.getValue(_loc8_) as TraxSample;
               _loc9_.setUsageFromSong(this.var_1784,_loc1_);
               if(_loc9_ == null)
               {
               ...
               while(_loc12_ < _loc11_)
               {
                  ...
                  _loc6_ += _loc10_;
                  _loc5_ = uint(_loc6_ * const_1418);
                  _loc12_++;
               }
               if(this.var_352 < _loc5_)
               {
                  this.var_352 = _loc5_;
               }
               _loc7_++;
      ...
      public function play(param1:Number = 0.0) : Boolean
      {
         ...
         this.var_513 = 0;
         this.var_618 = false;
         this.var_934.addEventListener(SampleDataEvent.SAMPLE_DATA,this.onSampleData);
         this.var_1428 = param1 * const_113;
         this.var_1155 = 0;
         this.var_1782 = 0;
         this.var_430 = this.var_934.play();
      ...
      {
         if(this.var_427 > 0 && !this.var_618)
         {
      ...
      private function stopImmediately() : void
      {
         ...
         {
            this.var_934.removeEventListener(SampleDataEvent.SAMPLE_DATA,this.onSampleData);
         }
      }
      ...
      private function stopWithFadeout() : void
      {
         if(this.var_619 == null)
         {
            this.var_749 = true;
            this.var_513 = 0;
            this.var_619 = new Timer(this.var_1429 + this.var_427 / (const_113 / 1000),1);
            this.var_619.start();
            this.var_619.addEventListener(TimerEvent.TIMER_COMPLETE,this.onFadeOutComplete);
         }
      }
      
      private function getSampleBars(param1:uint) : int
      {
         return Math.ceil(param1 / const_1417);
      }
      ...
      private function mixChannelsIntoBuffer() : void
      {
         ...
         while(_loc4_ >= 0)
         {
            ...
            _loc8_ = const_185;
            if(this.var_352 - this.var_205 < _loc8_)
            {
               _loc8_ = this.var_352 - this.var_205;
            }
            _loc9_ = 0;
            while(_loc9_ < _loc8_)
            {
      ...
      {
         var _loc1_:int = this.var_352 < this.var_1428 ? int(this.var_352) : (this.var_1428 > 0 ? int(this.var_1428) : int(this.var_352));
         if(this.var_205 > _loc1_ + this.var_1429 * (const_113 / 1000) && !this.var_618)
         {
            this.var_618 = true;
            if(this.var_429 != null)
            {
               this.var_429.reset();
               this.var_429.removeEventListener(TimerEvent.TIMER_COMPLETE,this.onPlayingComplete);
            }
            this.var_429 = new Timer(2,1);
            this.var_429.start();
            this.var_429.addEventListener(TimerEvent.TIMER_COMPLETE,this.onPlayingComplete);
         }
         else if(this.var_205 > _loc1_ - this.var_427 && !this.var_749)
         {
            this.var_933 = false;
            this.var_749 = true;
            this.var_513 = 0;
      ...
      {
         if(param1.position > this.var_1155)
         {
            ++this.var_1782;
            Logger.log("Audio buffer under run...");
            this.var_1155 = param1.position;
         }
         if(this.volume > 0)
         {
            this.mixChannelsIntoBuffer();
         }
         var _loc2_:int = const_185;
         if(this.var_352 - this.var_205 < _loc2_)
         {
            _loc2_ = this.var_352 - this.var_205;
            if(_loc2_ < 0)
            {
               _loc2_ = 0;
            }
         ...
         this.var_205 += const_185;
         this.var_1155 += const_185;
         if(this.var_430 != null)
         {
            this.var_1429 = param1.position / const_113 * 1000 - this.var_430.position;
         }
      ...
      private function writeAudioToOutputStream(param1:ByteArray, param2:int) : void
      {
         ...
         if(param2 > 0)
         {
            ...
            else
            {
               if(this.var_933)
               {
                  _loc5_ = 1 / this.var_748;
                  _loc6_ = this.var_932 / Number(this.var_748);
                  this.var_932 += const_185;
                  if(this.var_932 > this.var_748)
                  {
                     this.var_933 = false;
                  }
               }
               ...
               {
                  _loc5_ = -1 / this.var_427;
                  _loc6_ = 1 - this.var_513 / Number(this.var_427);
                  this.var_513 += const_185;
                  if(this.var_513 > this.var_427)
                  {
                     this.var_513 = this.var_427;
                  }
               }
      ...
      private function writeMixingBufferToOutputStream(param1:ByteArray, param2:int) : void
      {
         ...
         while(_loc4_ < param2)
         {
            _loc3_ = Number(Number(const_295[_loc4_]) * 0);
      ...
      private function writeMixingBufferToOutputStreamWithFade(param1:ByteArray, param2:int, param3:Number, param4:Number) : void
      {
         ...
         while(_loc6_ < param2)
         {
            ...
            _loc5_ = Number(Number(const_295[_loc6_]) * 0 * param3);
            param3 += param4;
         ...
         else if(param3 > 1)
         {
            while(_loc6_ < param2)
            {
               _loc5_ = Number(Number(const_295[_loc6_]) * 0);
               param3 += param4;
               param1.writeFloat(_loc5_);
               param1.writeFloat(_loc5_);
               _loc6_++;
      ...
      {
         this.stopImmediately();
         this._events.dispatchEvent(new SoundCompleteEvent(SoundCompleteEvent.TRAX_SONG_COMPLETE,this.var_1784));
      }
      ...
      private function removeFadeoutStopTimer() : void
      {
         if(this.var_619 != null)
         {
            this.var_619.removeEventListener(TimerEvent.TIMER_COMPLETE,this.onFadeOutComplete);
            this.var_619.reset();
            this.var_619 = null;
         }
      }
