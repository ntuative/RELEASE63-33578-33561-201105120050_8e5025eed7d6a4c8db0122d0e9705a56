package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1881:uint = 1;
      
      protected static const const_1883:uint = 2;
      
      protected static const const_1880:uint = 4;
      
      protected static const const_1882:uint = 3;
      
      protected static const name_4:uint = 7;
      
      public static const const_1871:int = 5;
      
      public static const LIBRARY_LOADER_FINALIZE:String = "LIBRARY_LOADER_FINALIZE";
      
      public static const const_1165:Boolean = false;
      
      public static const const_1624:int = 2;
      
      private static var var_943:Array = new Array();
      
      private static var var_756:Array = new Array();
      ...
      protected var var_2997:uint = 0;
      
      protected var var_2914:Boolean = true;
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1165)
         {
            ...
            while(_loc2_ > -1)
            {
               _loc1_ = var_756[_loc2_] as LibraryLoader;
               if(_loc1_ && (_loc1_.ready || _loc1_.disposed))
               {
                  var_756.splice(_loc2_,1);
               }
               _loc2_--;
            }
            while(var_756.length < const_1624 && false)
            {
               _loc1_ = var_943.shift() as LibraryLoader;
      ...
      {
         return !!this.var_1079 ? uint(this.var_1345 - this.var_2145) : uint(getTimer() - this.var_2145);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_405,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_43.close();
            }
      ...
      public function resume() : void
      {
         ...
         {
            this.var_1078 = false;
            if(!this.var_1079 && this.var_239)
            {
               this.var_1345 = -1;
               this.var_2145 = getTimer();
               queue(this);
            }
         }
      }
      ...
      {
         if(!this.var_1079 && !_disposed && this.var_1344 > 0)
         {
            try
            {
               this.var_43.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_239,const_1871 - this.var_1344);
            if(this.var_2914)
            {
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               ...
               this.debug("Load event STATUS " + this.var_405 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_405,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_405,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_405))
               {
                  this.var_1345 = getTimer();
         ...
         if(this.var_480 == LibraryLoader.const_1882)
         {
         ...
         if(this.var_480 == LibraryLoader.name_4)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_405,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_617 = this.getDefinition(this._name) as Class;
         ...
         try
         {
            xmlClass = this.var_617.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_405,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_405,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
