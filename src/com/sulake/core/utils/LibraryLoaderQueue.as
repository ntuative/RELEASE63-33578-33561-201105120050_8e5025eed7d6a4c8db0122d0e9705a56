package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1624:int = 4;
      ...
      {
         return this.var_473.length + this.var_287.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_287.length < const_1624 && this.var_473.length > 0)
            {
               _loc1_ = this.var_473.shift();
               this.var_287.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            ...
            index = this.var_287.indexOf(loader);
            if(index > -1)
            {
               this.var_287.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      public function findLibraryLoaderByURL(param1:String, param2:Boolean = true) : LibraryLoader
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_287)
            {
               if(param2)
               {
                  if(_loc3_.url.indexOf(param1) == 0)
