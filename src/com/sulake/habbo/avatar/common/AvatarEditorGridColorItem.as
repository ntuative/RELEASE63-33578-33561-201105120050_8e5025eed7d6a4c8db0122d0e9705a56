package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridColorItem
   {
      ...
      private var var_893:IPartColor;
      ...
      private function updateThumbData() : void
      {
         ...
         if(_loc2_ != null)
         {
            _loc2_.bitmap = new BitmapData(this.var_1053.width,this.var_1053.height,true,0);
            _loc5_ = this.var_1053.clone();
            _loc5_.colorTransform(_loc5_.rect,this.var_893.colorTransform);
         ...
         if(_loc3_ != null)
         {
            if(this._isSelected)
            {
               _loc3_.bitmap = new BitmapData(this.var_892.width,this.var_892.height,true,16777215);
               _loc3_.bitmap.copyPixels(this.var_892,this.var_892.rect,new Point(0,0),null,null,true);
               _loc3_.visible = true;
            }
            else
            {
         ...
         if(_loc4_)
         {
            if(this.var_893)
            {
               switch(this.var_893.clubLevel)
               {
                  case HabboClubLevelEnum.const_56:
                     _loc4_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_44:
                     _loc4_.style = HabboIconType.const_908;
                     _loc4_.visible = true;
                     break;
                  case HabboClubLevelEnum.const_47:
                     _loc4_.style = HabboIconType.const_969;
                     _loc4_.visible = true;
               }
            }
            else
