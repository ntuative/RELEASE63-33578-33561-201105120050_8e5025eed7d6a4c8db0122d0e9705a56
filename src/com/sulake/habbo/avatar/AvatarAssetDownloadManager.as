package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_1011:int = 3;
      
      private var var_456:AssetLoaderStruct;
      
      private var var_1013:Timer;
      ...
      private const const_2198:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_456 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_1013 = new Timer(this.const_2198,1);
         this.var_1013.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_456)
         {
            this.var_456.addEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_456.addEventListener(AssetLoaderEvent.const_49,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_456)
         {
            this.var_456.removeEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_456.removeEventListener(AssetLoaderEvent.const_49,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_1011);
            if(this.var_1534.indexOf("?") > 0)
            {
               _loc2_ = this.var_1534 + "&retry=" + this.var_1011;
            }
            else
            {
               _loc2_ = this.var_1534 + "?retry=" + this.var_1011;
            ...
            this.var_456 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_1958 = true;
         for each(_loc2_ in this.var_1238)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2696,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1239[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1958)
         {
            this.var_1238.push([param1,param2]);
            return;
         }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_1012)
         {
            _loc7_ = true;
            _loc4_ = this.var_1012[_loc3_];
      ...
      {
         var _loc1_:* = null;
         while(this.var_812.length > 0 && this.var_813.length < this.const_1624)
         {
            _loc1_ = this.var_812[0];
            _loc1_.startDownloading();
            this.var_813.push(this.var_812.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_812.indexOf(param1) == -1 && this.var_813.indexOf(param1) == -1)
         {
