package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_825:IFigurePartSet;
      ...
      private const const_1348:int = 13421772;
      
      private const const_1350:int = 16777215;
      
      private const const_367:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1984 = Math.max(this.var_1984,_loc6_.colorLayerIndex);
            }
         }
         this.var_458 = this.var_66.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_696 != null && !this.var_1985)
            {
               _loc3_ = this.var_696;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_825)
            {
               switch(this.var_825.clubLevel)
               {
                  case HabboClubLevelEnum.const_56:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_44:
                     _loc2_.style = HabboIconType.const_908;
                     break;
                  case HabboClubLevelEnum.const_47:
                     _loc2_.style = HabboIconType.const_969;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1216.color = this.const_1348;
         }
         else
         {
            this.var_1216.color = this.const_1350;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_367[_loc6_] + "_" + FigureData.const_153;
               _loc4_ = this.var_458.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_367.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_367[_loc6_] + "_" + FigureData.const_153;
                  _loc4_ = this.var_458.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_670 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1986)
               {
                  ...
                  this.var_1986 = _loc9_.content as BitmapData;
               }
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               if(this.var_1985 && _loc2_.colorLayerIndex > 0)
               {
